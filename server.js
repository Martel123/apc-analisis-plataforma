...
function fmt(s) { return s != null ? parseFloat(s).toFixed(1) : '—'; }
...
function makeCell(text, options = {}) {
  const { bold = false, color = '000000', shade = null, width = null } = options;
  const cellOptions = {
...
app.post('/api/comparison-docx', async (req, res) => {
  ...
  if (!ids || !Array.isArray(ids) || ids.length < 2) {
    return res.status(400).json({ ok: false, error: 'Se requieren al menos 2 ids de candidatos' });
  }
  ...
  if (candidates.length < 2) {
    return res.status(400).json({ ok: false, error: 'No se encontraron suficientes candidatos' });
  }

  const names = candidates.map(c => c.name).join(' vs ');
  const allBlockIds = [...new Set(candidates.flatMap(c => c.blocks.map(b => b.id)))];
  const blockNames = {};
  candidates.flatMap(c => c.blocks).forEach(b => { blockNames[b.id] = b.name; });
  ...
  sections.push(new Paragraph({
    ...
    children: [new TextRun({ text: 'Comparación técnica de planes de gobierno', bold: true, size: 36 })]
  }));
  sections.push(new Paragraph({
    children: [new TextRun({ text: names, color: '777777', size: 24 })]
  }));
  sections.push(new Paragraph({ children: [new TextRun({ text: 'Evaluación Técnica · Perú 2026', color: '999999', size: 20 })] }));
  sections.push(new Paragraph({ text: '' }));

  // ── Puntajes generales ────────────────────────────────────────────────────
  sections.push(new Paragraph({
    heading: HeadingLevel.HEADING_2,
    children: [new TextRun({ text: '1. Puntaje total por candidato', bold: true })]
  }));
  ...
  const scoreTable = new Table({
    ...
    rows: [
      ...
      ...sorted.map(c => new TableRow({
        children: [
          makeCell(c.name),
          makeCell(c.party || '—'),
          makeCell(fmt(c.total_score), { shade: scoreShade(c.total_score), bold: true }),
          makeCell(scoreLevelText(c.total_score), { shade: scoreShade(c.total_score) })
        ]
      }))
    ]
  });
  ...
  sections.push(new Paragraph({
    heading: HeadingLevel.HEADING_2,
    children: [new TextRun({ text: '2. Tabla comparativa por bloques', bold: true })]
  }));
  ...
  const blockTableRows = [
    new TableRow({
      ...
      children: [
        ...
        ...candidates.map(c => makeCell(c.name, { bold: true, shade: 'F3F4F6' })),
        makeCell('Líder', { bold: true, shade: 'F3F4F6' })
      ]
    }),
    ...allBlockIds.map(bid => {
      const scores = candidates.map(c => c.blocks.find(b => b.id === bid)?.average_score ?? null);
      const max = Math.max(...scores.filter(s => s !== null));
      const leaderIdx = scores.indexOf(max);
      return new TableRow({
        children: [
          makeCell(blockNames[bid] || bid),
          ...scores.map((s, i) => makeCell(fmt(s), { shade: scoreShade(s), bold: s === max && s !== null })),
          makeCell(leaderIdx >= 0 ? candidates[leaderIdx].name.split(' ')[0] : '—')
        ]
      });
    })
  ];
  sections.push(new Table({ width: { size: 9000, type: WidthType.DXA }, rows: blockTableRows }));
  sections.push(new Paragraph({ text: '' }));

  // ── Variables destacadas ──────────────────────────────────────────────────
  sections.push(new Paragraph({
    heading: HeadingLevel.HEADING_2,
    children: [new TextRun({ text: '3. Variables destacadas (top por candidato)', bold: true })]
  }));

  candidates.forEach(c => {
    const allVars = c.blocks.flatMap(b => b.variables.map(v => ({ ...v, blockName: b.name })));
    const top = [...allVars].filter(v => v.final_score != null).sort((a, b) => b.final_score - a.final_score).slice(0, 5);
    sections.push(new Paragraph({
    ...
    top.forEach(v => {
      sections.push(new Paragraph({
        bullet: { level: 0 },
        children: [new TextRun({ text: `${v.name} (${v.blockName}): ${fmt(v.final_score)}/10` })]
      }));
    });
    sections.push(new Paragraph({ text: '' }));
  });
  ...
  sections.push(new Paragraph({
    heading: HeadingLevel.HEADING_2,
    children: [new TextRun({ text: '4. Fortalezas por candidato', bold: true })]
  }));
  candidates.forEach(c => {
    const items = [...(c.strengths || []), ...(c.blocks || []).flatMap(b => b.strengths || [])].filter((s, i, a) => s && a.indexOf(s) === i).slice(0, 5);
    sections.push(new Paragraph({ children: [new TextRun({ text: c.name, bold: true, size: 24 })] }));
    if (items.length) {
      items.forEach(s => sections.push(new Paragraph({ bullet: { level: 0 }, children: [new TextRun({ text: s })] })));
    } else {
      sections.push(new Paragraph({ children: [new TextRun({ text: 'Sin fortalezas registradas', color: '999999' })] }));
    }
    sections.push(new Paragraph({ text: '' }));
  });
  ...
  sections.push(new Paragraph({
    heading: HeadingLevel.HEADING_2,
    children: [new TextRun({ text: '5. Debilidades por candidato', bold: true })]
  }));
  candidates.forEach(c => {
    const items = [...(c.weaknesses || []), ...(c.blocks || []).flatMap(b => b.weaknesses || [])].filter((s, i, a) => s && a.indexOf(s) === i).slice(0, 5);
    sections.push(new Paragraph({ children: [new TextRun({ text: c.name, bold: true, size: 24 })] }));
    if (items.length) {
      items.forEach(s => sections.push(new Paragraph({ bullet: { level: 0 }, children: [new TextRun({ text: s })] })));
    } else {
      sections.push(new Paragraph({ children: [new TextRun({ text: 'Sin debilidades registradas', color: '999999' })] }));
    }
    sections.push(new Paragraph({ text: '' }));
  });
  ...
  sections.push(new Paragraph({
    heading: HeadingLevel.HEADING_2,
    children: [new TextRun({ text: '6. Conclusión comparativa', bold: true })]
  }));
  ...
  allBlockIds.forEach(bid => {
    ...
    candidates.forEach(c => {
      ...
      if (s !== null && s > bestS) { best = c; bestS = s; }
    });
    if (best) blockWins[best.id] = (blockWins[best.id] || 0) + 1;
  });
  const mostWins = candidates.reduce((b, c) => ((blockWins[c.id]||0) > (blockWins[b?.id]||0)) ? c : b, candidates[0]);
